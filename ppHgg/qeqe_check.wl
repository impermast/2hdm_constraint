...
diags = InsertFields[CreateTopologies[0, 2 -> 2], {F[3, {1}], F[2, {1}]} -> 
        {F[3, {1}], F[2, {1}]}, InsertionLevel -> {Classes}, Model -> "SMQCD", 
...
SetMandelstam[s, t, u, p1, p2, -k1, -k2, SMP["m_u"], 0, SMP["m_u"],0];



ampQA[1] = 1/SUNN^2(ampQA[0] (ComplexConjugate[ampQA[0]])) // 
...
ampQA[2] = ampQA[1] // ReplaceAll[#, {SMP["m_u"] -> 0,SMP["m_e"] -> 0}] & // 
...
MatrixPrefactor = Simplify[Matrix[s,Th]/((1/4 (1-Cos[Th])^2-(1-Cos[Th])+2)/(1-Cos[Th])^2)]
...
Int = Integrate[Sin[Th]*(1/4 (1-Cos[Th])^2-(1-Cos[Th])+2)/(1-Cos[Th])^2,{Th,Th1,Th2}];
...
(*dSigma/dt = 1/(16 pi s^2) M^2*)
(**)
(*x = pt/E =  sin theta = sqrt(1-cos th^2) = > 1-cos^2 th = (pt/E)^2 => cos th = sqrt(1 - (pt/E)^2 ) = sqrt(1 - (4pt^2/s))*)
(*4E^2=s => E^2 = s/4*)
(*(pp->qq )*)
...
th1 = ArcCos[ Sqrt[1-4 pta^2/s0]] // N
th2 = 0
Tlim2[x_]:= x*s0/(Exp[2 y1]+1)
Tlim1[x_]:= x*s0/(Exp[2 y2]+1)
Sqrt[Tlim1[1]]//N
Sqrt[Tlim2[1]]//N
...
IntPbT = SigmaT * 3.8* 10^8 
IntPbT2 = SigmaTwithY * 3.8*10^8


Print["Cross t, pt cuts: ",IntPbT," pb.       ( MG cross = 2.47*10^5 pb   Y cuts MGcross = ??pb );   Cross = ", SigmaTwithY * 4* 10^8]
Abs[IntPbT-40.4]
